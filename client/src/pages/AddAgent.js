...
const AddAgent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const agentResponse = await axios.post("http://localhost:5000/api/agents/add", formData)
      
      if (agentResponse.status === 201) {
        if (file) {
          const formDataFile = new FormData()
          formDataFile.append("file", file)
          const uploadResponse = await axios.post("http://localhost:5000/api/upload/upload-file", formDataFile, {
